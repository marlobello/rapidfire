...
interface ScoreDao {
    ...
    @Query("SELECT * FROM scores WHERE gameMode != 'BOSS_RUSH' ORDER BY score DESC LIMIT 10")
    ...
    @Query("DELETE FROM scores WHERE gameMode != 'BOSS_RUSH' AND id NOT IN (SELECT id FROM scores WHERE gameMode != 'BOSS_RUSH' ORDER BY score DESC LIMIT 10)")
    suspend fun pruneStandardScores()

    @Query("DELETE FROM scores WHERE gameMode = 'BOSS_RUSH' AND id NOT IN (SELECT id FROM scores WHERE gameMode = 'BOSS_RUSH' ORDER BY score DESC LIMIT 10)")
    suspend fun pruneBossRushScores()
}
